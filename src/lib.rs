        ...
        OUT: OutputDataProducer,
{
    ...
    pub fn collect_out_frames(&mut self, now: Instant, out: &mut VecDeque<Frame>) {
        ...
        if segments.len() == 0 {
            self.next_frame_id += 1;
            let frame = self.create_out_frame(now, reliability, 0, 0, &[0; 0]);
            out.push_back(frame);
        }
        ...
        for (segment_number, segment_data) in segments.enumerate() {
            self.next_frame_id += 1;
            let frame = self.create_out_frame(now, reliability, count_segments, segment_number, segment_data);
            out.push_back(frame);
        }
        self.next_packed_id += 1;
    }

    fn create_out_frame(&mut self, now: Instant, reliability: bool, count_segments: usize, segment_number: usize, segment_data: &[u8]) -> Frame {
...
pub mod tests {
    ...
    #[test]
    fn should_keep_alive_frame_create() {
        let mut protocol = create_protocol(5);
