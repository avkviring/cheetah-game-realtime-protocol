...
impl RoundTripTime {
    ...
    pub fn on_frame_received(&mut self, frame: &Frame, now: Instant) {
        ...
        if frame.headers.first(|p| match p {
            Header::Retransmit(header) => Some(header),
            _ => None
        }).is_some() {
            return;
        }
        ...
        let request_header: Option<&RoundTripTimeHeader> = frame.headers.first(|p| match p {
            Header::RoundTripTimeRequest(header) => Some(header),
            _ => None
        });
        ...
        let response_header: Option<&RoundTripTimeHeader> = frame.headers.first(|p| match p {
            Header::RoundTripTimeResponse(header) => Some(header),
            _ => None
        });
...
mod tests {
    ...
    use crate::frame::Frame;
    use crate::frame::headers::Header;
    use crate::others::rtt::{AVERAGE_RTT_MIN_LEN, RoundTripTime, RoundTripTimeHeader};
    ...
    pub(crate) fn should_ignore_retransmit_frame_when_receive_request() {
        ...
        assert!(matches!(output_frame.headers.first(|p| match p {
			Header::RoundTripTimeResponse(header) => Some(header),
			_ => None
		}), None));
    }
