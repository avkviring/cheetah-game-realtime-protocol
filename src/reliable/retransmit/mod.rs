...
impl Retransmitter {
	...
	pub(crate) fn on_frame_received(&mut self, frame: &Frame) {
		let ack_headers: HeaderVec<&AckHeader> = frame.headers.find(|p| match p {
			Header::Ack(header) => Some(header),
			_ => None
		});
...
mod tests {
	...
	fn should_add_retransmit_header() {
		...
		assert!(matches!(
			...
			if frame.frame_id == 2
			&&
			frame.headers.first(|p| match p {
            Header::Retransmit(header) => Some(header),
            _ => None
        }).unwrap().original_frame_id==original_frame.frame_id
		));
	}
